...
SELECT c.id, c.nombre, c.fecha_inicio, c.fecha_fin, c.ubicacion
FROM competicion c
JOIN nacionalidad n ON c.codpais = n.codpais
WHERE n.nombre_pais = 'Estados Unidos';
...
SELECT d.nombre AS disciplina,
       MIN(r.resultado) AS mejor_tiempo,
       MAX(r.resultado) AS peor_tiempo
FROM resultados r
...
tiempo obtenido  para el evento con valor 2 . Descarta aquellos atletas que no tengan tiempo .*/
